	...
	var Inputs = {
		...
		createTextInput: function (elem) {
			...
			if(!elem.created){
				...
				newField = document.getElementById(elem.id + "_");
			}
			$newField = $(newField);
	...
	valid = {
		...
		ipv6suffix: function (str) {
			...
			if (isNaN(value)) {
				...
				return _("Value range: %s - %s", [0, 128]);
			}
		},
		...
		ipv6: {
			...
			specific: function (str) {
				...
				if (headStr.slice(0, 2) == "ff") {//组播地址ff00::/8开始
					return (_("Please input a valid address."));
				}
				...
				if (idx == -1) {
					...
					if (str == "::" || str == "::1") {//相当于0.0.0.0 || 127.0.0.1
						return (_("Please input a valid address."));
					}
